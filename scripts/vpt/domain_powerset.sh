#!/bin/bash
cd ../../
use_domain_cls_loss=$1
use_nearest_neighbor_loss=$2
is_divided=$3
...
for ((i = 1; i < 15; i++)); do
  ...
  bash scripts/vpt/main.sh 1 domainnet_mini_df 1 vit_b16_ep50 8 9 $use_domain_cls_loss $use_nearest_neighbor_loss $is_divided $selected_domains_str
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/vpt/main.sh 1 pacs_df 1 vit_b16_ep50 8 9 $use_domain_cls_loss $use_nearest_neighbor_loss $is_divided $selected_domains_str
  # echo "${selected_domains[@]}"
done
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/vpt/main.sh 1 office_home_df 1 vit_b16_ep50 8 9 $use_domain_cls_loss $use_nearest_neighbor_loss $is_divided $selected_domains_str
  # echo "${selected_domains[@]}"
done