...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'IVLP_VL_Adapter_Local_SelectPatch_FullMask',
                      ...
                      "select_layer": cfg.TRAINER.IVLP_VLADAPTER_LOCAL_SELECTPATCH.SELECT_LAYER
                      }
...
class CustomCLIP(nn.Module):
    ...
    def encode_image(self, image):
        image_features, local_feat, image_features_masked, local_feat_masked = self.image_encoder(image.type(self.dtype))
        pass
...
class IVLP_VL_Adapter_Local_SelectPatch_FullMask(TrainerDF_Local_SelectPatch):
    ...
    def build_model(self):
        ...
        model_expert = load_clip_to_cpu_expert(cfg)
        model_expert.cuda()
        model_expert.eval()

        with torch.no_grad():
            text_features = model_expert.encode_text(prompts)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        self.text_features_expert = text_features
