...
USE_TEXT_ADAPTER=${16}
USE_SOFT_DOMAIN_LABEL=${17}
PREPROCESS_SOFT_LABEL=${18}
LOSS_WEIGHT=${19}
UPDATEEPOCH=${20}

SUBEXPNAME=DC-${USE_DOMAIN_CLS_LOSS}_UpdEpoch${UPDATEEPOCH}_SoftPreprocess${PREPROCESS_SOFT_LABEL}_LossW${19}_NN-${USE_NEAREST_NEIGHBOR_LOSS}_DIV-${IS_DOMAIN_DIVIDED}_InstPG-${USE_CROSSATTENTION}
...
python train_loop.py \
    ...
    USE_SOFT_DOMAIN_LABEL ${USE_SOFT_DOMAIN_LABEL} \
    PREPROCESS_SOFT_LABEL ${PREPROCESS_SOFT_LABEL} \
    SOFT_LABEL_UPDATE_EPOCH ${UPDATEEPOCH} \
    DDL_LOSS_WEIGHT ${LOSS_WEIGHT}
