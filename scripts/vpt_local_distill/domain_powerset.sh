...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/vpt_local_distill/main.sh 1 pacs_df 1 vit_b16_ep50 8 9 $selected_domains_str
  # echo "${selected_domains[@]}"
done
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/vpt_local_distill/main.sh 0 office_home_df 1 vit_b16_ep50 8 9 $selected_domains_str
  # echo "${selected_domains[@]}"
done
# domains=("art" "clipart" "product" "real_world")
# for ((i = 1; i < 15; i++)); do
