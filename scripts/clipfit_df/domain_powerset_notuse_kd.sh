...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/clipfit_df/domain_forgetting_notuse_kd.sh $gpu_id office_home_df $seed vit_b16_ep50 $shots $selected_domains_str
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/clipfit_df/domain_forgetting_notuse_kd.sh $gpu_id domainnet_mini_df $seed vit_b16_ep50 $shots $selected_domains_str
  # bash scripts/vpt_with_dc/main.sh 0 domainnet_mini_df 1 vit_b16_ep50 8 9 $selected_domains_str
  # echo "${selected_domains[@]}"
done

bash scripts/clipfit_df/domain_forgetting_notuse_kd.sh $gpu_id visda17_df $seed vit_b16_ep50 $shots synthetic
bash scripts/clipfit_df/domain_forgetting_notuse_kd.sh $gpu_id visda17_df $seed vit_b16_ep50 $shots real
