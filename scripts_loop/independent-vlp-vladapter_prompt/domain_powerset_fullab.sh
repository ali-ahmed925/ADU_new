...
for USE_DOMAIN_CLS_LOSS in true false; do
    for USE_NEAREST_NEIGHBOR_LOSS in true false; do
        for IS_DOMAIN_DIVIDED in true false; do
            for USE_CROSSATTENTION in true false; do
                # サブエクスペリメント名の設定
                ...
                bash scripts_loop/independent-vlp-vladapter_prompt/domain_forgetting_fullab.sh $CUDA_DEVICE $DATASET $SEED $CFG $NCTX $DEPTH_VISION $DEPTH_TEXT \
                    ...
                    $SHOTS $EXPNAME $USE_CROSSATTENTION
            done
        done
    done
done
