...
#   bash scripts/vpt_w_nnl_local/main.sh 1 office_home_df 1 vit_b16_ep50 8 9 25 $selected_domains_str
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/vpt_w_prompt_generator/main.sh 0 pacs_df 1 vit_b16_ep50 8 9 3 $selected_domains_str
  # echo "${selected_domains[@]}"
done
...
#   # コマンドを実行
#   selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
#   bash scripts/vpt_w_nnl_local/main.sh 1 domainnet_mini_df 1 vit_b16_ep50 8 9 54 $selected_domains_str
