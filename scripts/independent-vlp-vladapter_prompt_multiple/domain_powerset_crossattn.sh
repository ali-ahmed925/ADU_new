...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/independent-vlp-vladapter_prompt_multiple/domain_forgetting_crossattn_${bashname}.sh $gpu_id office_home_df $seed vit_b16_ep50 8 $vision_depth $text_depth $use_domain_cls_loss $use_nearest_neighbor_loss $is_divided $shots $selected_domains_str
...
for ((i = 1; i < 15; i++)); do
  ...
  # コマンドを実行
  selected_domains_str=$(IFS=" "; echo "${selected_domains[*]}")
  bash scripts/independent-vlp-vladapter_prompt_multiple/domain_forgetting_crossattn_${bashname}.sh $gpu_id domainnet_mini_df $seed vit_b16_ep50 8 $vision_depth $text_depth $use_domain_cls_loss $use_nearest_neighbor_loss $is_divided $shots $selected_domains_str
...
# bash scripts/independent-vlp-vladapter_prompt_multiple/domain_forgetting_crossattn_${bashname}.sh $gpu_id visda17_df $seed vit_b16_ep50 8 $vision_depth $text_depth $use_domain_cls_loss $use_nearest_neighbor_loss false $shots synthetic
# bash scripts/independent-vlp-vladapter_prompt_multiple/domain_forgetting_crossattn_${bashname}.sh $gpu_id visda17_df $seed vit_b16_ep50 8 $vision_depth $text_depth $use_domain_cls_loss $use_nearest_neighbor_loss false $shots real
