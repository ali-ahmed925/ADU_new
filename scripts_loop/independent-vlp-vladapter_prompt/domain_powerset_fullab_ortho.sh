    ...
    for USE_NEAREST_NEIGHBOR_LOSS in false; do
        for IS_DOMAIN_DIVIDED in false true; do
            for USE_CROSSATTENTION in true; do
                ...
                # 実行コマンド
                bash scripts_loop/independent-vlp-vladapter_prompt/domain_forgetting_fullab_ortho.sh $CUDA_DEVICE $DATASET $SEED $CFG $NCTX $DEPTH_VISION $DEPTH_TEXT \
                    $USE_DOMAIN_CLS_LOSS $USE_NEAREST_NEIGHBOR_LOSS $IS_DOMAIN_DIVIDED \
                    $SHOTS $EXPNAME $USE_CROSSATTENTION $DATASETSEED $USE_VISION_ADAPTER $USE_TEXT_ADAPTER $USE_ORTHOGONAL_LOSS
            done
        done
    done
done

DATASET=domainnet_mini_df
